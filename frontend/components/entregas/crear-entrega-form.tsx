...
import {
  obtenerCantidadPorProgramar,
  obtenerLugarEntregaDetallePedido,
} from "@/services/detalle-pedido/utils/detalle-pedido.util";
import { Alert } from "../ui/alert";
import {
  formatFecha,
  obtenerFechaEntregaSiguiente,
} from "@/lib/utils";
...
export function CrearEntregaForm({ pedido }: { pedido: PedidoEntity }) {
  ...
  const detallesPedidosPendientesPorDespachar = pedido.detallesPedido
    .map((detalle: DetallePedidoEntity) => {
      return {
        ...detalle,
        cantidadDespachar: obtenerCantidadPorProgramar(
          detalle,
          pedido.entregas
        ),
      };
  ...
  async function onSubmit() {
    ...
    const dataEntrega: CrearEntregaModel = {
      ...values,
      pedidoId: pedido.id,
      entregasProducto: values.productos
        .map((p) => ({
          ...p,
    ...
    const response = await new EntregaPedidoService(
      session.data?.user.token || ""
    ).crearEntrega(dataEntrega);
    if (!response.data) {
      return toast.toast({
        title: "Error",
  ...
  const hayProductosProgramdas =
    form
      .getValues()
      .productos.filter((p) => p.cantidadDespachar >= 0 && p.incluir)?.length >
    0;
  const hayProductosPorProgramar =
    form.getValues().productos.filter((p) => p.cantidadDespachar >= 0)?.length >
    0;
  const handleSubmit = async () => {
    if (!hayProductosProgramdas) {
      toast.toast({
        variant: "destructive",
        description:
          "La cantidad a despachar de los productos debe ser mayor a cero.",
      });
      return;
    }
    await onSubmit();
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
          <Card>
            <CardHeader>
              ...
              <CardDescription>
                Datos generales para la programación de la entrega
              </CardDescription>
            </CardHeader>
          ...
          <Card>
            <CardHeader>
              ...
              <CardDescription>
                Selecciona los productos y cantidades a incluir en esta entrega
              </CardDescription>
            </CardHeader>
            <CardContent>
                ...
                {!hayProductosPorProgramar && (
                  <Alert>
                    <p className="text-sm text-muted-foreground">
                      ✅ Todos los productos han sido completamente programados
                      para entrega.
                    </p>
                    <p className="mt-2 text-sm">
                      Si aún no has completado las entregas anteriores, por
                      favor finalízalas para poder programar nuevos productos.
                    </p>
                  </Alert>
                )}
                {hayProductosPorProgramar &&
                  detallesPedidosPendientesPorDespachar.map(
                    (detalle: DetallePedidoEntity, index: number) => {
                      const lugarEntrega =
                        obtenerLugarEntregaDetallePedido(detalle);
                      const cantidadPorProgramar = obtenerCantidadPorProgramar(
                        detalle,
                        pedido.entregas
                      );
                      return (
                        ...
                        >
                          ...
                          <div className="flex flex-row flex-wrap space-x-2 space-y-2">
                            ...
                            <InfoItem
                              ...
                              value={formatFecha(detalle.fechaEntrega)}
                            />
                          </div>
                              ...
                              <FormItem>
                                ...
                                <FormControl>
                                  <Input
                                    ...
                                    disabled={
                                      !form.watch(`productos.${index}.incluir`)
                                    }
                                  />
                                </FormControl>
                                <FormDescription>
                                  Cantidad a incluir en esta entrega (máximo:{" "}
                                  {detalle.cantidad -
